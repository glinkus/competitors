...
class FullSiteSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        "PLAYWRIGHT_BROWSER_TYPE": "chromium",
        "PLAYWRIGHT_LAUNCH_OPTIONS": {"headless": True},
        "CONCURRENT_REQUESTS": 1000,  # Adjust for stability (reduce concurrent requests)
        "DOWNLOAD_DELAY": 1, 
        "DOWNLOAD_HANDLERS": {
            "http": "scrapy_playwright.handler.ScrapyPlaywrightDownloadHandler",
            "https": "scrapy_playwright.handler.ScrapyPlaywrightDownloadHandler"
    ...
    def spider_opened(self):
        def fetch_urls():
            ...
            with open("/home/gustas/indeform_praktika/marketing/osom-codex/dev/modules/competitors_scraper/competitors_scraper/spiders/log.txt", "a") as log_file:
                log_file.write(f"URLs from DB: {self.urls}\n")
    ...
    def start_requests(self):
        # We schedule initial requests in spider_opened.
        return iter([])

    def update_page_sync(self, page, response):
        close_old_connections()
        print("Updating page")
        print(f"Page urlblet: {page.url}")
        page.visited = True
    ...
    def get_page_sync(self, website_id, url):
        ...
        print("Getting page for:", norm_url)
